...
# ─── EXPORTAR / IMPORTAR EXCEL COMPLETO ──────────────────────────────────────

@app.get("/exportar-excel-completo")
def exportar_excel_completo(token=Depends(require_admin)):
    """Exporta todas las tablas del sistema en un Excel (una hoja por tabla)."""
    import io
    import openpyxl
    ...
    try:
        ...
        for table in tables:
            if table in ("alembic_version",):
                continue
            rows = db.execute(text(f'SELECT * FROM "{table}"')).fetchall()
            keys = list(db.execute(text(f'SELECT * FROM "{table}" LIMIT 0')).keys())
            # Nombre de hoja (max 31 chars para Excel)
            sheet_name = table[:31]
            ws = wb.create_sheet(title=sheet_name)
            # Header
            for col_idx, col_name in enumerate(keys, 1):
            ...
            for row_idx, row in enumerate(rows, 2):
                for col_idx, val in enumerate(row, 1):
                    cell_val = str(val) if val is not None and not isinstance(val, (int, float, bool)) else val
                    ws.cell(row=row_idx, column=col_idx, value=cell_val)
            # Ajustar ancho de columnas
            for col_idx, col_name in enumerate(keys, 1):
                ws.column_dimensions[openpyxl.utils.get_column_letter(col_idx)].width = max(len(col_name) + 4, 12)
    finally:
        db.close()
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f'attachment; filename="bbcfile_backup_{ts}.xlsx"'},
    )


@app.post("/importar-excel-completo")
async def importar_excel_completo(
    file: UploadFile = File(...),
    token=Depends(require_admin),
):
    """Importa un Excel completo (una hoja por tabla) y restaura la base de datos."""
    import openpyxl
    from database import SessionLocal
    from sqlalchemy import inspect, text
    ...
    try:
        ...
        for sheet_name in wb.sheetnames:
            # Buscar tabla: nombre exacto o match parcial (por truncamiento a 31 chars)
            table_name = sheet_name
            if table_name not in existing_tables:
                match = [t for t in existing_tables if t[:31] == sheet_name]
                if match:
                    table_name = match[0]
                else:
                    errors.append(f"Tabla '{sheet_name}' no existe, saltada")
                    continue

            ws = wb[sheet_name]
            rows_iter = ws.iter_rows(values_only=True)
            try:
                columns = [str(c) for c in next(rows_iter) if c is not None]
            ...
            try:
                ...
                for row_vals in rows_iter:
                    vals = list(row_vals[:len(columns)])
        ...
        # Limpiar duplicados afiliados/eliminados
    ...
    return {
        ...
        "restaurado": restored,
        "errores": errors,
        "mensaje": "Se creó un backup de seguridad antes de importar",
    }

...
def reporte_cobro(
    ...
    ws = wb.active
    ws.title = "Cobro"
    cols = ["#", "Nombre", "Documento", "Empresa", "Cliente", "Día cobro", "Servicios", "Planilla ($)", "Estado"]
    _hdr_style(ws, cols)
    for i, r in enumerate(items, 1):
        srvs = r.get("servicios") or []
        srvs_str = ", ".join(srvs) if isinstance(srvs, list) else str(srvs)
        ws.append([i, r.get("nombre"), r.get("doc"), r.get("empresa"), r.get("cliente"),
...
def reporte_afiliados(
    ...
    ws = wb.active
    ws.title = "Afiliados"
    cols = ["#", "Nombre", "Documento", "Empresa", "Cliente", "Cargo", "EPS", "ARL", "AFP", "CCF",
            "Subtipo", "Estado", "Estado Servicio", "Servicios", "IBC", "Tel", "Email", "Fecha Ingreso", "Fecha Afiliación", "Obs"]
    _hdr_style(ws, cols)
    for i, a in enumerate(items, 1):
        srvs = a.get("servicios") or []
        srvs_str = ", ".join(srvs) if isinstance(srvs, list) else str(srvs)
        ws.append([i, a.get("nombre"), a.get("doc"), a.get("empresa"), a.get("cliente_txt"),
...
def reporte_retiros(
    ...
    items = crud.get_retiros(db, anio=anio, mes=mes)
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Retiros"
    cols = ["#", "Nombre", "Documento", "Empresa", "Fecha", "Motivo", "Mes", "Año", "Observaciones", "Registrado por"]
    _hdr_style(ws, cols)
    for i, r in enumerate(items, 1):
        ws.append([i, r.get("nombre"), r.get("doc"), r.get("empresa"), r.get("fecha"),
                   r.get("motivo"), r.get("mes"), r.get("anio"), r.get("obs"), r.get("registrado_por")])
    for col in ws.columns:
        ws.column_dimensions[col[0].column_letter].width = max(len(str(col[0].value or "")), 12)
...
def reporte_consolidado(
    ...
    cols2 = ["#", "Nombre", "Documento", "Empresa", "Cliente", "EPS", "ARL", "AFP", "CCF", "Estado", "Servicios", "IBC"]
    _hdr_style(ws2, cols2)
    for i, a in enumerate(afiliados, 1):
        srvs = a.get("servicios") or []
        srvs_str = ", ".join(srvs) if isinstance(srvs, list) else str(srvs)
        ws2.append([i, a.get("nombre"), a.get("doc"), a.get("empresa"), a.get("cliente_txt"),
    ...
    cols3 = ["#", "Código", "Afiliado", "Cliente", "Mes", "Año", "Ingresos", "Planilla", "Utilidad", "Estado"]
    _hdr_style(ws3, cols3)
    for i, f in enumerate(facturas, 1):
        ws3.append([i, f.get("codigo"), f.get("nombre_afiliado"), f.get("cliente"),
                    f.get("mes"), f.get("anio"), f.get("ingresos", 0),

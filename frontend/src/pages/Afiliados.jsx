...
import DocumentosTab from '../components/afiliados/DocumentosTab';
...
export default function Afiliados() {
  ...
  const erroresCoherencia = useMemo(() => {
    ...
    const epsOpts = [...(listas.eps || [])];
    if (form.eps && !epsOpts.includes(form.eps)) epsOpts.push(form.eps);
    const tieneEps = set_(form.eps, epsOpts);
  ...
  return (
    <div>
      ...
      <Modal open={!!modal} onClose={()=>setModal(null)} width={720}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0 16px' }}>
          <Sel label="EPS"        value={form.eps||''} onChange={v=>sf('eps',v)}
            options={[''].concat(listas.eps||[]).concat(form.eps && !(listas.eps||[]).includes(form.eps) ? [form.eps] : [])} />
      ...
      <ConsultaSS
        ...
        onAplicar={campos=>setForm(f=>{
          const next = { ...f, ...campos };
          if (campos.eps) {
            const srv = [...(next.servicios || [])];
            if (!srv.includes('EPS')) srv.push('EPS');
            next.servicios = srv;
          }
          return next;
        })}
